  ...
  return (
    <div className={className}>
      ...
      {!file ? (
        ...
        >
          <div className={`rounded-lg border p-3 transition ${dragging ? "border-primary/30 bg-primary/10" : "border-white/10 bg-white/5"}`}>
        ...
        /* File preview */
        <div className="flex items-center gap-4 rounded-lg border border-white/10 bg-gradient-to-b from-white/5 to-white/[0.01] p-4">
          {previewUrl ? (
            <img
              src={previewUrl}
              alt="Preview"
      ...
      {/* Error */}
      {error && (
        <p className="mt-2 rounded-lg border border-rose-500/20 bg-rose-500/10 px-4 py-2.5 text-base font-semibold text-rose-400">
          {error}
        </p>
      )}
    </div>
