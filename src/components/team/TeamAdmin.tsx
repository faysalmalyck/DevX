...
type TeamForm = Omit<
  TeamMemberRecord,
  "id" | "legacyDepartment" | "profileStatus" | "createdAt" | "updatedAt"
>;
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <button onClick={openCreate} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-brand px-5 text-base font-bold text-white shadow-sm transition hover:bg-brand">
      ...
      <div className="flex flex-col items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#111827] sm:flex-row">
        <div className="flex w-full flex-1 gap-3 sm:w-auto">
          <div className="relative w-full max-w-sm">
            ...
            <input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search by name or role..." className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-10 pr-4 text-base outline-none focus:border-brand dark:border-white/10 dark:bg-zinc-900" />
          </div>
          <select aria-label="Filter by department" value={departmentFilter} onChange={(event) => setDepartmentFilter(event.target.value)} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-base dark:border-white/10 dark:bg-zinc-900">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
          <div role="dialog" aria-modal="true" aria-labelledby="delete-team-member-title" className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl dark:bg-[#111827]">
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button disabled={deleting} onClick={() => setDeleteTarget(null)} className="rounded-xl px-4 py-2 font-bold text-slate-600 dark:text-zinc-300">Cancel</button>
              <button disabled={deleting} onClick={() => void deleteMember()} className="rounded-xl bg-rose-600 px-4 py-2 font-bold text-white disabled:opacity-60">{deleting ? "Deleting…" : "Delete"}</button>
            </div>
          </div>
        </div>
      ) : null}
...
function StatCard({ icon, label, value, accent = "blue" }: { icon: ReactNode; label: string; value: number; accent?: "blue" | "emerald" }) {
  return <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#111827]"><div className="flex items-center gap-3"><div className={`flex h-10 w-10 items-center justify-center rounded-xl ${accent === "emerald" ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10" : "bg-blue-50 text-brand dark:bg-brand/10"}`}>{icon}</div><div><p className="text-base font-bold text-slate-500 dark:text-zinc-400">{label}</p><p className="text-2xl font-black text-slate-900 dark:text-white">{value}</p></div></div></div>;
}
