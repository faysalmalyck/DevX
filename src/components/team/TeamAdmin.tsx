  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <button onClick={openCreate} className="consultation-primary-btn px-5 py-2.5 text-sm font-semibold">
          <Plus className="mr-1.5 h-4 w-4 inline" />Add Member
        </button>
      </div>
      ...
      <div className="flex flex-col items-center justify-between gap-4 cart-skin-box rounded-2xl p-4 sm:flex-row">
        <div className="flex w-full flex-1 gap-3 sm:w-auto">
          <div className="relative w-full max-w-sm">
            <Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
            <input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search by name or role..." className="consultation-input pl-10 text-sm" />
          </div>
          <select aria-label="Filter by department" value={departmentFilter} onChange={(event) => setDepartmentFilter(event.target.value)} className="consultation-select text-sm py-2.5 px-4">
            <option value="All">All Departments</option>
            {TEAM_MEMBER_DEPARTMENTS.map((department) => <option key={department.value} value={department.value}>{department.label}</option>)}
          </select>
        </div>
        <div className="flex gap-1 rounded-lg bg-slate-100 p-1 dark:bg-[#1a2031]">
          <button onClick={() => setViewMode("list")} className={`rounded-md p-1.5 ${viewMode === "list" ? "bg-white shadow-sm dark:bg-[#262d43]" : "text-slate-500"}`} aria-label="List view"><ListIcon className="h-4 w-4" /></button>
          <button onClick={() => setViewMode("grid")} className={`rounded-md p-1.5 ${viewMode === "grid" ? "bg-white shadow-sm dark:bg-[#262d43]" : "text-slate-500"}`} aria-label="Grid view"><GridIcon className="h-4 w-4" /></button>
        </div>
      </div>

      {filteredMembers.length === 0 ? (
        <div className="flex flex-col items-center justify-center cart-skin-box rounded-3xl py-16">
      ...
      {deleteTarget ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
          <div role="dialog" aria-modal="true" aria-labelledby="delete-team-member-title" className="cart-skin-box w-full max-w-md rounded-2xl p-6">
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button disabled={deleting} onClick={() => setDeleteTarget(null)} className="consultation-secondary-btn px-5 py-2.5 text-sm font-semibold">Cancel</button>
              <button disabled={deleting} onClick={() => void deleteMember()} className="inline-flex items-center justify-center rounded-full bg-rose-600 px-6 py-2.5 text-sm font-semibold text-white shadow-[0_8px_20px_rgba(225,29,72,0.25)] transition-all duration-200 hover:-translate-y-0.5 hover:bg-rose-700 hover:shadow-[0_0_20px_rgba(225,29,72,0.4)] active:translate-y-0 disabled:opacity-60">{deleting ? "Deleting…" : "Delete"}</button>
            </div>
          </div>
        </div>
      ) : null}
...
function StatCard({ icon, label, value, accent = "blue" }: { icon: ReactNode; label: string; value: number; accent?: "blue" | "emerald" }) {
  return <div className="cart-skin-box rounded-xl p-5"><div className="flex items-center gap-3"><div className={`flex h-10 w-10 items-center justify-center rounded-lg ${accent === "emerald" ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10" : "bg-blue-50 text-brand dark:bg-brand/10"}`}>{icon}</div><div><p className="text-base font-bold text-slate-500 dark:text-zinc-400">{label}</p><p className="text-2xl font-black text-slate-900 dark:text-white">{value}</p></div></div></div>;
}
...
function MemberCard({ member, onEdit, onDelete }: { member: TeamMemberRecord; onEdit: (member: TeamMemberRecord) => void; onDelete: (member: TeamMemberRecord) => void }) {
  return <div className="cart-skin-box group relative flex flex-col items-center rounded-2xl p-6"><div className="absolute right-4 top-4 flex gap-1"><button aria-label={`Edit ${displayName(member)}`} onClick={() => onEdit(member)} className="rounded-full bg-slate-100 p-2 text-brand dark:bg-zinc-800"><Pencil className="h-4 w-4" /></button><button aria-label={`Delete ${displayName(member)}`} onClick={() => onDelete(member)} className="rounded-full bg-slate-100 p-2 text-rose-500 dark:bg-zinc-800"><Trash2 className="h-4 w-4" /></button></div><img src={member.image || imageFallback(member.name)} alt="" onError={(event) => { event.currentTarget.src = imageFallback(member.name); }} className="h-24 w-24 rounded-full object-cover" /><h2 className="mt-4 text-lg font-bold text-slate-900 dark:text-white">{displayName(member)}</h2><p className="text-base text-brand dark:text-brand">{member.role || "Role not set"}</p><span className="mt-2 text-xs text-slate-500">{displayDepartment(member)}</span><div className="mt-4 flex flex-wrap items-center justify-center gap-2"><ProfileStatusBadge status={member.profileStatus} /><PublicationStatus status={member.status} /></div></div>;
}
...
function MemberModal({
  ...
  const input = "mt-1.5 consultation-input text-sm";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
      <form
        onSubmit={onSubmit}
        noValidate
        className="cart-skin-box flex max-h-full w-full max-w-2xl flex-col overflow-hidden rounded-3xl"
      >
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <Field label="Access & Role" field="accessRole" required error={fieldError("accessRole")}>
              <select
                ...
                className={`mt-1.5 consultation-select text-sm ${fieldError("accessRole") ? "border-rose-500 focus:border-rose-500" : ""}`}
              >
            ...
            <Field label="Department" field="department" required error={fieldError("department")}>
              <select id="team-member-department" value={draft.department ?? ""} onChange={(event) => onChange("department", (event.target.value || null) as TeamForm["department"])} className={`mt-1.5 consultation-select text-sm ${fieldError("department") ? "border-rose-500 focus:border-rose-500" : ""}`}>
            ...
            <Field label="Public status" field="status" error={fieldError("status")}>
              <select id="team-member-status" value={draft.status} onChange={(event) => onChange("status", event.target.value as TeamForm["status"])} className={`mt-1.5 consultation-select text-sm ${fieldError("status") ? "border-rose-500 focus:border-rose-500" : ""}`}>
                <option value="DRAFT">Draft</option>
                <option value="PUBLISHED">Published</option>
              </select>
            </Field>
            <label className="flex items-center gap-3 text-sm font-semibold text-slate-700 dark:text-[#c9d0e1] sm:col-span-2">
              <input aria-label="Feature this member" type="checkbox" checked={draft.featured} onChange={(event) => onChange("featured", event.target.checked)} className="h-4 w-4 rounded accent-brand" />
              Feature this member
            </label>
            <Field label="Biography (directory teaser)" field="bio" required error={fieldError("bio")} wide>
              <textarea id="team-member-bio" rows={4} value={draft.bio ?? ""} onChange={(event) => onChange("bio", event.target.value || null)} className={`consultation-textarea mt-1.5 text-sm ${fieldError("bio") ? "border-rose-500 focus:border-rose-500" : ""}`} />
            </Field>
            <Field label="About" field="about" error={fieldError("about")} wide>
              <textarea id="team-member-about" rows={6} value={draft.about ?? ""} onChange={(event) => onChange("about", event.target.value || null)} className={`consultation-textarea mt-1.5 text-sm ${fieldError("about") ? "border-rose-500 focus:border-rose-500" : ""}`} />
            </Field>
            <Field label="About Paragraph 2" field="aboutParagraph2" error={fieldError("aboutParagraph2")} wide>
              <textarea id="team-member-aboutParagraph2" rows={6} value={draft.aboutParagraph2 ?? ""} onChange={(event) => onChange("aboutParagraph2", event.target.value || null)} className={`consultation-textarea mt-1.5 text-sm ${fieldError("aboutParagraph2") ? "border-rose-500 focus:border-rose-500" : ""}`} />
            </Field>
            <HighlightsEditor
            ...
            <Field label="Experience" field="experience" error={fieldError("experience")} wide>
              <textarea id="team-member-experience" rows={6} value={draft.experience ?? ""} onChange={(event) => onChange("experience", event.target.value || null)} className={`consultation-textarea mt-1.5 text-sm ${fieldError("experience") ? "border-rose-500 focus:border-rose-500" : ""}`} />
            </Field>
          </div>
        ...
        <div className="flex justify-end gap-3 border-t border-slate-200 bg-slate-100/60 px-6 py-4 dark:border-slate-700/60 dark:bg-black/20">
          <button type="button" onClick={onClose} disabled={submitting} className="consultation-secondary-btn px-6 py-2.5 text-sm font-semibold">Cancel</button>
          <button disabled={submitting} className="consultation-primary-btn px-7 py-2.5 text-sm font-semibold">{submitting ? "Saving…" : "Save Profile"}</button>
        </div>
      </form>
    </div>
  );
...
function HighlightsEditor({
  ...
  return (
    <fieldset className="sm:col-span-2">
      <legend className="text-sm font-semibold text-slate-700 dark:text-[#c9d0e1]">Highlights</legend>
      <p className="mt-1 text-xs font-normal text-slate-500 dark:text-zinc-400">Add ordered bullet points for this member’s profile page.</p>
        ...
        {highlights.map((highlight, index) => (
          <div key={index} className="flex items-center gap-2">
            ...
            <div className="flex shrink-0 gap-1">
              <button type="button" aria-label={`Move highlight ${index + 1} up`} disabled={index === 0} onClick={() => moveHighlight(index, index - 1)} className="consultation-secondary-btn px-2.5 py-1 text-xs disabled:opacity-40">↑</button>
              <button type="button" aria-label={`Move highlight ${index + 1} down`} disabled={index === highlights.length - 1} onClick={() => moveHighlight(index, index + 1)} className="consultation-secondary-btn px-2.5 py-1 text-xs disabled:opacity-40">↓</button>
              <button type="button" aria-label={`Remove highlight ${index + 1}`} onClick={() => onChange(highlights.filter((_, currentIndex) => currentIndex !== index))} className="inline-flex items-center justify-center rounded-full border border-rose-200 dark:border-rose-500/30 bg-white dark:bg-[#1e2538] px-3 py-1 text-xs font-semibold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10">Remove</button>
            </div>
          </div>
        ))}
      </div>
      <button type="button" onClick={() => onChange([...highlights, ""])} className="consultation-secondary-btn mt-3 px-4 py-2 text-xs font-semibold">
...
function Field({ label, field, required = false, error, wide = false, children }: { label: string; field: string; required?: boolean; error?: string; wide?: boolean; children: ReactNode }) {
  ...
  return (
    <div className={`text-sm font-semibold text-slate-700 dark:text-[#c9d0e1] ${wide ? "sm:col-span-2" : ""}`}>
      <label htmlFor={id} className="mb-1 block">
        {label}
        {required ? <span className="ml-1 text-rose-600" aria-hidden="true">*</span> : null}
      </label>
      {children}
      {error ? <p id={`${id}-error`} role="alert" className="mt-1 text-xs font-medium text-rose-600 dark:text-rose-300">{error}</p> : null}
    </div>
  );
}
