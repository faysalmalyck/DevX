...
function salesOnly(data: z.infer<typeof teamMemberSchema>) {
...
export async function GET() {
  ...
  try {
    ...
    return json({ members: members.map((member) => ({ ...serializeTeamMember(member), admin: member.admin ? { ...member.admin, lastLogin: member.admin.lastLogin?.toISOString() ?? null } : null })) });
  } catch (error) {
